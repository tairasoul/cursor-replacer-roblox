...
setInterval(async () => {
    ...
    if (isRCREnabled && fs.existsSync("C:\\RCOC\\selected.json")) {
        ...
        if (fs.existsSync(`${locapp}\\Roblox\\Versions`)) {
            ...
            const curhashes = await utils.getMD5s({path: selected["Arrow"], name: "Arrow"}, {path: selected["ArrowFar"], name: "ArrowFar"}, {path: selected["IBeam"], name: "IBeam"});
            const oldhashes = await utils.getMD5s({path: `${rv}\\ArrowCursor.png`, name: "Arrow"}, {path: `${rv}\\ArrowFarCursor.png`, name: "ArrowFar"}, {path: `${rv}\\IBeamCursor.png`, name: "IBeam"});
